using System;           //Reno addition
using System.Collections;
using System.Collections.Generic;
using System.IO;        //Reno addition
using UnityEngine;
...
namespace BNG {
    public class VehicleController : MonoBehaviour {
        ...
        public int incrementer = 0;         //RENO class GLOBAL VARIABLE
        ...
        public virtual void GetTorqueInputFromTriggers() {
            ...
            if(isHoldingSteering) {
                SetMotorTorqueInput((InputBridge.Instance.RightTrigger * 150.0f) - (InputBridge.Instance.LeftTrigger * 250.0f)); //Reno added * #.0f, old setting was 50 & 150
            }
        ...
        void OnTriggerEnter(Collider collider)
        {
            ...
            {
                var path = @"D:\Unity\Unity Projects\NudgesResearch\CheckpointSpeeds.csv";
                string enterSpeed = CurrentSpeed.ToString("n0");

                using (StreamWriter sw = File.AppendText(path))
                {
                    sw.WriteLine(otherObj.name + "," + enterSpeed);
                }

                //string[] connection = {otherObj.name, enterSpeed};
                //File.AppendAllLines(path, connection);
